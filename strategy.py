...
# Calcul des signaux BUY et SELL avec un contrôle strict pour éviter la surabondance
buy_signals = []
sell_signals = []
...
for i, (date, price) in enumerate(zip(data.index, data["Close"])):
    ...
    # Déterminer la position actuelle par rapport aux bornes
    if price <= lower_bound:  # En dessous de la limite inférieure
    ...
    elif price >= upper_bound:  # Au-dessus de la limite supérieure
